...
| 收录版本 | `0.8.5` |
| 上游 commit | `fd87f36`(Merge PR #16 plugin_check release hygiene; tag `v0.8.5`) |
| License | MIT |
| 功能 | 模型回答中直接渲染 `dsh-ui` 围栏:布局 / 图表 / 表单 / quiz / mermaid / 3D,以及 `render_ui` 工具 |

## 收录说明

- 快照取自上游 `main`(`fd87f36` / v0.8.5),排除 `.git`、`node_modules`。
...
```bash
git clone --depth 1 https://github.com/omdsh-dev/dsh-genui.git <tmp>
robocopy <tmp> third-party/dsh-genui /MIR /XD .git node_modules /XF UPSTREAM.md
