...
| 收录版本 | `0.1.0` |
| 上游 commit | `25f21ae`(docs: restore the Wire Think-Execute Standard section, drop dead mode refs) |
| License | MIT |
| 功能 | 首轮用官方 Minimal 真实工具对(`bash` + `str_replace_editor`)锚定轨迹,晋升后走小型 resident 目录;另含 `zero-anchored-standard/` 与 `whoami-standard/` 变体 |

## 收录说明

- git subtree 跟随上游 `main`(`25f21ae`,`--squash` 模式;版本号保持 0.1.0)。
- **未修改任何上游文件**。合集侧只多这一份 `UPSTREAM.md`。
- `scripts/install-anchored-standard.ps1` 只安装基础模式 `preset/`;上游 HEAD 另含
  `zero-anchored-standard/`、`whoami-standard/`、`wire-think-standard/` 等变体,
  需单独复制到 `~/.dsh/.agent-presets/` 才会出现。
