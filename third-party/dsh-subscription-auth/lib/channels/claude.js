...
const RESOLVE_THRESHOLD_MS = 60000;
const defaultModels = [
  { id: "claude-opus-4-8", name: "Claude Opus 4.8", contextWindow: 400000 },
...
async function exchangeClaudeCode(code, port, verifier, state) {
  ...
  create(ctx) {
    ...
    return {
      ...
      async login() {
        ...
        pending = { url };
        waitForCallback(port, state, controller.signal, 10 * 60 * 1000, CALLBACK_PATH).then(async (code) => {
      ...
      async discoverModels() {
        const token = await ctx.readToken();
        if (!token || token.expires - Date.now() < RESOLVE_THRESHOLD_MS)
          return [];
        try {
